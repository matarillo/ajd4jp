...
public class LunisolarYear implements Comparable<LunisolarYear>, java.io.Serializable {
	...
	private static HashMap<Integer, LunisolarYear>	map = new HashMap<Integer, LunisolarYear>();
	...
	private static Saku[] countMon( int year, Saku[] saku, int start, int end ) {
		...
		for ( ; i < saku.length; i++ ) {
			...
			else	saku[i - 1].leap_f = set_f = true;
			saku[i - 1].year = year;
			saku[i - 1].mon = mm;
		}
		saku[i - 1].year = year;
	...
	private static int seekToji(Saku[] saku, int test, AJD toji) {
		if (!saku[test].ajd.equals(toji)) { return -1; }
		for (int i = test; i >= 0; i--) {
			if (saku[i].start_f) { return i; }
		}
		return -1;
	}
	...
	private static LunisolarYear get11( AJD ajd ) {
		...
		for ( int i = 0; i < toji.length; i++ ) {
			toji[i] = STCD.getStart( yyyy, START ).trim();
			yyyy++;
			if ( yyyy == 0 )	yyyy++;
		}
		int	day = new Span( toji[0], toji[3] ).getDayPart();
		Saku[]	saku = new Saku[day + 1];
		for ( int i = 0; i < saku.length; i++ )	saku[i] = new Saku();
		saku[0].ajd = toji[0];
		saku[0].sun = START;
		saku[day].ajd = toji[3];
		saku[day].sun = START;
		...
		for( int i = 0; i < line[0].length; i++ ) {
			if ( line[0][i].year == year )	list.add( line[0][i] );
		}
		for ( int i = 0; i < line[1].length; i++ ) {
			list.add( line[1][i] );
		}
		for ( int i = 0; i < line[2].length; i++ ) {
			if ( line[2][i].year != year )	break;
			list.add( line[2][i] );
		}
		int	cnt = list.size();
		for ( int i = 1; i < cnt; i++ ) {
	...
	public static LunisolarYear getLunisolarYear( AJD hint ) {
		...
		synchronized( map ) {
			for ( int i = 0; i < 3; i++, year++ ) {
				...
				if ( y != null ) {
					if ( y.getLSCD( hint ) != null )	return y;
				}
			}
			return get11( hint );
		}
	...
	private LunisolarYear( int y, ArrayList<Saku> list ) {
		...
		for ( ; start < cnt; start++ ) {
			if ( list.get( start ).year == y )	break;
		}
		for ( ; start < cnt; start++ ) {
	 ...
	 * @return ajdに対応する旧暦の日付。年の範囲外であればnull。
	 */
	public LSCD getLSCD( AJD dd ) {
		...
		for ( ; i < month.length; i++ ) {
			if ( ajd.compareTo( month[i].ajd ) < 0 )	break;
		}
		return new LSCD(
	...
	public int getLeapMonth() {
		for ( int i = 0; i < month.length; i++ ) {
			if ( month[i].leap_f )	return month[i].mon;
		}
		return -1;
	}
	...
	public LSCD getLSCD( int mm, boolean leap_f, int dd, int hh, int mi, int ss ) throws AJDException {
		int last = getLastDay( mm, leap_f );
		if ( dd < 1 || dd > getLastDay( mm, leap_f ) )	throw new AJDException( "指定日が存在しません。" );
		Saku[]	mon = getSaku( mm, leap_f );
	...
	public boolean equals( Object o ) {
		if ( o instanceof LunisolarYear )	return yy == ((LunisolarYear)o).yy;
		return false;
	}
}
